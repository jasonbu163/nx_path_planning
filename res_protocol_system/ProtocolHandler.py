# res_protocol_system/ProtocolHandler.py
from res_protocol_system import RESProtocol, PacketBuilder, PacketParser
import asyncio
import logging
...
class ProtocolHandler:
        ...
        def __init__(self, host, port, device_id=1):
            ...
            self.latest_status = {
                'location': (0, 0, 0),
                'car_status': RESProtocol.CarStatus.READY,
                'battery': 100,
                'last_update': None
            }
            ...
            while self._running and self.connected:
                try:
                    ...
                    if self._heartbeat_counter % 5 == 0:
                        frame_type = RESProtocol.FrameType.HEARTBEAT_WITH_BATTERY
                    else:
        ...
        def _update_status(self, status_data):
            ...
            if 'result' in status_data and status_data['result'] != 0:
                ...
                error_info = RESProtocol.ErrorHandler.get_error_info(error_code)
                ...
                # 如果是关键错误，触发紧急处理
                if RESProtocol.ErrorHandler.is_critical_error(error_code):
        ...
        async def send_emergency_stop(self):
            ...
            return await self.send_command(
                cmd_id=RESProtocol.CommandID.EMERGENCY_STOP.value,
