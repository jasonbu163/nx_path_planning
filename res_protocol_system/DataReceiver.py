...
import time
import asyncio
from res_protocol_system import PacketParser, RESProtocol
...
class DataReceiver:
    ...
    def _receive_loop(self):
        ...
        while self.running:
            ...
            # elif frame_type == RESProtocol.FrameType.DATA:
    ...
    def _handle_task_response(self, data):
        ...
        # 处理错误代码
        if result_code != RESProtocol.ErrorHandler.ERROR_MAP[0]:
    ...
    def _handle_command_response(self, data):
        ...
        # 特殊处理急停命令
        if cmd_id == RESProtocol.CommandID.EMERGENCY_STOP:
            if result == RESProtocol.ErrorHandler.ERROR_MAP[0]:
                print("急停命令已成功执行")
                # 更新所有任务状态为停止
    ...
    def _handle_error(self, error_code):
        """统一错误处理"""
        error_name, solution = RESProtocol.ErrorHandler.get_error_info(error_code)
