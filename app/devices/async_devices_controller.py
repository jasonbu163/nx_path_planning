# app/devices/async_devices_controller.py
import asyncio
import logging
...
class AsyncDevicesController():
    ...
    async def car_cross_layer(
        ...
        logger.info(f"🧭 穿梭车目的楼层: {target_layer} 层")
        ...
        if await self.car.car_current_location() != car_current_lift_pre_location:
            ...
            if await self.car.car_move(task_no+1, car_current_lift_pre_location):
                # 等待穿梭车移动到位
                logger.info(f"⏳ 等待穿梭车前往 5,3,{car_current_floor} 位置...")
                ...
                if await self.car.car_current_location() == car_current_lift_pre_location and car_status['car_status'] == CarStatus.READY.value:
                    logger.info(f"✅ 穿梭车已到达 {car_current_lift_pre_location} 位置")
                else:
                    logger.error(f"❌ 穿梭车未到达 {car_current_lift_pre_location} 位置")
        ...
        if await self.car.car_current_location() != car_current_lift_location:
            ...
            if await self.car.car_move(task_no+2, car_current_lift_location):
                # 等待穿梭车进入电梯
                logger.info(f"⏳ 等待穿梭车前往 电梯内 6,3,{car_current_floor} 位置...")
                ...
                if await self.car.car_current_location() == car_current_lift_location and car_status['car_status'] == CarStatus.READY.value:
                    logger.info(f"✅ 穿梭车已到达 电梯内 {car_current_lift_location} 位置")
                else:
                    logger.error(f"❌ 穿梭车未到达 电梯内 {car_current_lift_location} 位置")
                    return [False, "❌ 穿梭车运行错误"]
            else:
                logger.error(f"❌ 穿梭车未到达 电梯内 {car_current_lift_location} 位置")
                return [False, "❌ 穿梭车运行错误"]
        ...
        if await self.plc.async_connect() and self.plc.plc_checker():
            ...
            if self.plc.get_lift() == target_layer and self.plc.read_bit(11, DB_11.IDLE.value) == 1:
                ...
                logger.info(f"✅ 穿梭车位置: {car_target_lift_location}")
            else:
                await self.plc.async_disconnect()
                logger.error("❌ 电梯未到达")
                return [False, "❌ 电梯未到达"]
        ...
        logger.info(f"🚧 穿梭车开始离开电梯进入接驳位 {target_lift_pre_location}")
        
        logger.info("⏳ 穿梭车开始移动...")
        await self.car.car_move(task_no+5, target_lift_pre_location)
        
        # 等待穿梭车进入接驳位
        logger.info(f"⏳ 等待穿梭车前往 接驳位 {target_lift_pre_location} 位置...")
        ...
        if await self.car.car_current_location() == target_lift_pre_location:
            logger.info(f"✅ 穿梭车已到达 指定楼层 {target_layer} 层")
        else:
            logger.error(f"❌ 穿梭车未到达 指定楼层 {target_layer} 层")
            return [False, "❌ 穿梭车运行错误"]
    ...
    async def task_inband(
        ...
        car_layer = car_loc[2]
        logger.info(f"🚗 穿梭车当前楼层: {car_layer}")
        # 拆解目标位置 -> 坐标: 如, "1,3,1" 楼层: 如, 1
        logger.info(f"📦 货物目标坐标: {TARGET_LOCATION}")
        target_loc = list(map(int, TARGET_LOCATION.split(',')))
        target_layer = target_loc[2]
        logger.info(f"📦 货物目标楼层: {target_layer}")
        ...
        await asyncio.sleep(1)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 移动电梯载货到目标楼层 {target_layer}层")
            await asyncio.sleep(1)
        ...
        if await self.car.car_current_location() != car_current_lift_pre_location:
            ...
            # 等待穿梭车移动到位
            logger.info(f"⏳ 等待穿梭车前往 5,3,{target_layer} 位置...")
            await self.car.wait_car_move_complete_by_location(car_current_lift_pre_location)
            await asyncio.sleep(2)
        ...
        await asyncio.sleep(1)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 穿梭车开始取货...")
            await asyncio.sleep(1)
            self.plc.pick_in_process(target_layer)
            await self.plc.async_disconnect()
        else:
            await self.plc.async_disconnect()
            logger.error("❌ PLC接收取货信号异常")
            return [False, "❌ PLC接收取货信号异常"]
        
        # 穿梭车将货物移动到目标位置
        logger.info(f"🚧 穿梭车将货物移动到目标位置 {TARGET_LOCATION}")
        logger.info("⏳ 穿梭车开始移动...")
        await self.car.good_move(TASK_NO+4, TARGET_LOCATION)
        
        # 等待穿梭车进入接驳位
        logger.info(f"⏳ 等待穿梭车前往 {TARGET_LOCATION} 位置...")
        ...
        if await self.car.car_current_location() == TARGET_LOCATION:
            logger.info(f"✅ 货物已到达 目标位置 {TARGET_LOCATION}")
        else:
            logger.error(f"❌ 货物未到达 目标位置 {TARGET_LOCATION}")
            return [False, "❌ 穿梭车运行错误"]
    ...
    async def task_outband(
        ...
        # 拆解目标位置 -> 坐标: 如, "1,3,1" 楼层: 如, 1
        logger.info(f"📦 目标货物坐标: {TARGET_LOCATION}")
        target_loc = list(map(int, TARGET_LOCATION.split(',')))
        target_layer = target_loc[2]
        logger.info(f"📦 目标货物楼层: {target_layer}")
        ...
        await asyncio.sleep(1)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 移动空载电梯到 {target_layer} 层")
            await asyncio.sleep(2)
        ...
        logger.info(f"▶️ 出库开始")

        # 穿梭车将货物移动到目标位置
        logger.info(f"🚧 穿梭车前往货物位置 {TARGET_LOCATION}")
        if await self.car.car_current_location() != TARGET_LOCATION:
            logger.info("⏳ 穿梭车开始移动...")
            ...
            # if await self.car.car_current_location() == TARGET_LOCATION and car_status['car_status'] == CarStatus.READY.value:
            if await self.car.car_current_location() == TARGET_LOCATION:
                logger.info(f"✅ 穿梭车已到达 货物位置 {TARGET_LOCATION}")
            else:
                logger.error(f"❌ 穿梭车未到达 货物位置 {TARGET_LOCATION}")
                return [False, "❌ 穿梭车运行错误"]
        ...
        # 穿梭车将货物移动到楼层接驳位
        target_lift_pre_location = f"5,3,{target_layer}"
        logger.info(f"🚧 穿梭车将货物移动到楼层接驳位输送线 {target_lift_pre_location}")
        logger.info("⏳ 穿梭车开始移动...")
        await self.car.good_move(TASK_NO+3, target_lift_pre_location)
        
        # 等待穿梭车进入接驳位
        logger.info(f"⏳ 等待穿梭车前往 {target_lift_pre_location} 位置...")
        await self.car.wait_car_move_complete_by_location(target_lift_pre_location)
        await asyncio.sleep(2)
        ...
        # if await self.car.car_current_location() == target_lift_pre_location and car_status['car_status'] == CarStatus.READY.value:
        if await self.car.car_current_location() == target_lift_pre_location:
            logger.info(f"✅ 货物已到达 楼层接驳输送线位置 {target_lift_pre_location}")
        else:
            logger.error(f"❌ 货物未到达 楼层接驳输送线位置 {target_lift_pre_location}")
            return [False, "❌ 穿梭车运行错误"]
...
class DevicesControllerByStep():
    ...
    async def action_lift_move_backup(
        ...
        Args:
            ...
            LAYER: int
        """
        logger.info(f"▶️ 电梯开始移动到{LAYER}层...")
    ...
    async def action_lift_to_outband(self) -> list:
        ...
        logger.info("🚧 电梯-出口输送线启动...")

        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info("📦 货物开始进入出库口...")
            await asyncio.sleep(2)
    ...
    async def action_lift_to_everylayer(self, TARGET_LAYER: int) -> list:
        """[动作 - 电梯-楼层输送线] 货物从电梯输送线进入楼层输送线。

        Args:
            TARGET_LAYER: 目标楼层
        """
        logger.info(f"🚧 {TARGET_LAYER}层电梯-{TARGET_LAYER}输送线启动...")

        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"📦 货物开始进入{TARGET_LAYER}层...")
            await asyncio.sleep(2)
            self.plc.lift_to_everylayer(TARGET_LAYER)

            logger.info(f"⏳ {TARGET_LAYER}层输送线移动中...")
    ...
    async def action_pick_in_process(self, TARGET_LAYER: int) -> list:
        ...
        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 发送{TARGET_LAYER}层取货进行中信号...")
            await asyncio.sleep(2)
    ...
    async def action_pick_complete(self, TARGET_LAYER: int) -> list:
        ...
        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 发送{TARGET_LAYER}层取货完成信号...")
            await asyncio.sleep(2)
    ...
    async def action_feed_in_process(self, TARGET_LAYER: int) -> list:
        ...
        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 发送{TARGET_LAYER}层放货进行中信号...")
            await asyncio.sleep(2)
    ...
    async def action_feed_complete(self, TARGET_LAYER: int) -> list:
        """[动作 - 取货完成 - 出库] 发送放货完成信号给PLC

        Args:
            TARGET_LAYER: 目标层
        """
        logger.info(f"🚧 {TARGET_LAYER}楼层-电梯输送线启动...")

        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            logger.info(f"🚧 发送{TARGET_LAYER}层放货完成信号...")
            await asyncio.sleep(2)
            if self.plc.feed_complete(TARGET_LAYER):
                logger.info(f"✅ 发送{TARGET_LAYER}层放货完成信号成功")
    
                logger.info(f"⏳ {TARGET_LAYER}层接驳位和电梯输送线移动中...")
                await self.plc.wait_for_bit_change(11, DB_11.PLATFORM_PALLET_READY_1020.value, 1)
    ...
    async def action_car_move(self, TASK_NO: int, TARGET_LOCATION: str) -> list:
        """[动作] 移动车辆。
    ...
    async def action_good_move(
        ...
        """
        logger.info(f"⏳ 穿梭车前往需要移动货物 {SOURCE_LOCATION} 处...")
        move_car_info =  await self.action_car_move(TASK_NO, SOURCE_LOCATION)
        if move_car_info[0]:
            logger.info(f"✅ {move_car_info[1]}")
        else:
            logger.error(f"❌ {move_car_info[1]}")
            return [False, f"❌ {move_car_info[1]}"]
        
        logger.info(f"⏳ 移动货物正在前往 {TARGET_LOCATION} 处...")
        await asyncio.sleep(1)
        if await self.car.car_current_location() != TARGET_LOCATION:
            ...
            if await self.car.good_move(TASK_NO+1, TARGET_LOCATION):
                ...
                logger.info(f"⏳ 等待货物前往 {TARGET_LOCATION} 位置...")
                await self.car.wait_car_move_complete_by_location(TARGET_LOCATION)
    ...
    async def comb_change_car_location(self, TASK_NO: int, TARGET_LAYER: int) -> list:
        ...
        logger.info(f"▶️ 正在获取电梯层号...")

        await asyncio.sleep(2)
        if await self.plc.async_connect() and self.plc.plc_checker():
            await asyncio.sleep(3)
            if self.plc.get_lift() == TARGET_LAYER and self.plc.read_bit(11, DB_11.IDLE.value) == 1:
                logger.info(f"🚧 电梯到达{TARGET_LAYER}, 开始更新穿梭车位置...")
                await self.plc.async_disconnect()
    ...
    async def car_cross_layer(self, TASK_NO: int, TARGET_LAYER: int) -> list:
        ...
        # 穿梭车先进入电梯口，不直接进入电梯，要避免冲击力过大造成危险
        car_current_lift_pre_location = f"5,3,{car_current_floor}"
        logger.info(f"🚧 移动空载电梯到电机口 {car_current_lift_pre_location}...")

        car_move_info = await self.action_car_move(TASK_NO+1, car_current_lift_pre_location)
        if car_move_info[0]:
            logger.info(f"{car_move_info[1]}")
        else:
            logger.error(f"{car_move_info[1]}")
        ...
        # 穿梭车进入电机
        car_current_lift_location = f"6,3,{car_current_floor}"
        logger.info(f"🚧 穿梭车进入电梯内 {car_current_lift_location} ...")
        ...
        logger.info(f"🚧 移动电梯载车到{TARGET_LAYER}层...")
        ...
        logger.info(f"🚧 更新电梯内穿梭车车到{TARGET_LAYER}层位置...")
        
        comb_plc_car_info = await self.comb_change_car_location(TASK_NO+4, TARGET_LAYER)
        if comb_plc_car_info[0]:
            logger.info(f"{comb_plc_car_info[1]}")
        else:
            logger.error(f"{comb_plc_car_info[1]}")
        ...
        logger.info(f"🚧 校准电梯{TARGET_LAYER}层水平位置...")
    ...
    async def task_inband(self, TASK_NO: int, TARGET_LOCATION: str) -> list:
        """[任务入库] 穿梭车系统联合PLC电梯输送线系统, 执行入库任务。
        ...
        if good_move_info[0]:
            logger.info(f"{good_move_info[1]}")
        else:
            logger.error(f"{good_move_info[1]}")
        ...
        logger.info(f"🚧 电梯载货到目标楼层 {target_layer}层...")
        ...
        # 电梯载货到到目标楼层, 电梯输送线将货物送入目标楼层
        logger.info(f"🚧 货物进入 {target_layer}层...")
        ...
        logger.info(f"🚧 发送{target_layer}层取货进行中信号给PLC...")
        ...
        car_current_lift_pre_location = f"5,3,{target_layer}"
        logger.info(f"🚧 穿梭车移动 {car_current_lift_pre_location} 货物到 {TARGET_LOCATION} ...")
        ...
        logger.info(f"🚧 发送{target_layer}层取货完成信号给PLC...")
    ...
    async def task_outband(self, TASK_NO: int, TARGET_LOCATION: str) -> list:
        """[任务出库] 穿梭车系统联合PLC电梯输送线系统, 执行出库任务
        ...
        # 电梯初始化: 移动到目标货物层
        logger.info(f"🚧 电梯移动到目标层 {target_layer} 层")
        ...
        logger.info(f"🚧 发送{target_layer}层放货进行中信号给PLC...")
        ...
        target_lift_pre_location = f"5,3,{target_layer}"
        logger.info(f"🚧  穿梭车移动 {TARGET_LOCATION} 货物到 {target_lift_pre_location} ...")
        ...
        logger.info(f"🚧 发送{target_layer}层放货完成信号给PLC...")
