...
from .car_connection_module import CarConnectionBase, CarConnection, AsyncCarConnection
...
class AsyncCarController(CarConnection):
    """
    ...
    async def send_heartbeat(self, TIMES: int=3) -> dict:
        """
        [发送心跳包] - 心跳报文可以获取穿梭车状态
    ...
    async def car_power(self, TIMES: int=3) -> Any:
        """
        [获取穿梭车电量] - 发送电量心跳包，获取穿梭车电量
    ...
    async def car_status(self, TIMES: int=3) -> dict:
        """
        [获取穿梭车状态] - 发送心跳报文，获取穿梭车状态信息
...
class AsyncSocketCarController(AsyncCarConnection):
    """
    [穿梭车 - 高级操作类] 基于asyncio封装
    """
