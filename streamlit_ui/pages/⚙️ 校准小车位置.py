...
steps = [
    ...
    },
    {
        "step": 2,
        "title": "🚗 获取小车位置",
        "api": "/api/v1/wcs/control/get_car_location",
        "method": "GET",
        "params": {}
    },
]
...
for i, step in enumerate(steps):
    ...
    if step["step"] == 1:
        with st.expander(step["title"], expanded=True):
            user_inputs = {}
            if step["api"] == "/api/v1/wcs/control/change_car_location":
                for key, default in step["params"].items():
                    st.markdown("**小车目标坐标**（x=行, y=列, z=层）")
                    col1, col2 = st.columns(2)
                    with col1:
                        x = st.selectbox("小车行号 (x)", list(range(1, 7)), key=f"{key}_x_{i}")
                    with col2:
                        y = st.selectbox("小车列号 (y)", list(range(1, 9)), key=f"{key}_y_{i}")
        ...
        with st.expander(step["title"], expanded=True):
            ...
            if st.button(f"🚀 [执行] {step['title']}", key=f"exec_{i}"):
                try:
                    ...
                    try:
                        car_loc = resp.json()["data"]
                        st.markdown(f"**小车位置：{car_loc}**")
                    except:
                        st.text(resp.text)
